...
test('cul-de-sac gardens redistribute existing shrubs into all three clear house frontages', () => {
  ...
  assert.equal(frontageGardenPlacements({ ...manifest, barriers: manifest.barriers.filter(b => !b.id.startsWith('activity-yucca-')) }).length, 141,
    'retain the original shrub layout before accounting for new solid yuccas');
  assert.equal(placements.length, 115, '26 shrubs yield to the new collidable yuccas instead of overlapping them');
