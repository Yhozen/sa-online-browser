...
The user revised the cloud software-rendering target to **5 FPS per view**. P95 frame time remains reported; the former 100 ms cap is retired because a 5 FPS median corresponds to a 200 ms frame. Hardware GPU performance must be measured separately; these cloud numbers are functional smoke-test measurements.
...
- The unrecorded two-view cloud benchmark completed 11 active driving/reset/role rounds after warm-up, including independent server/peer agreement checks. It measured **5.0 / 4.6 FPS median**, with **266.6 / 250.1 ms p95** at full 1280×720 resolution. The second view is below the strict 5 FPS target, so the performance test remains failing; it has not been rounded up or represented as passing.
...
The benchmark still enforces >=5 FPS, full native resolution and asset/geometry/texture budgets. A missed FPS target fails the test without reducing rendering quality. The full summary now expects 15 browser scenarios, including the added native-resolution regression.
