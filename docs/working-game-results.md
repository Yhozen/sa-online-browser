...
- **Merged-build checks:** typecheck, 3 gateway tests, 108 unit/asset tests, 1 activity DOM test, 3 native CTests and headed audio passed. The focused two-browser run drove both nine-gate laps in **56,881 ms** and **50,996 ms** with swapped roles, then exercised false starts, exits, reset and rejoin. It ended with **SIGTERM / exit 143** during the final Standard screenshot, before final report and cleanup assertions; its cause is unconfirmed. This is an interrupted run, not a full acceptance pass. See [delivery evidence](working-game-verification.json). Earlier results/agreements files were not overwritten and are explicitly excluded from merged-build proof.

![Actual merged-build second lap, Low preset at native 1280×720](images/arroyo-activity-merged.png)
