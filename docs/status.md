...
Dream-loop implementation is in progress; see [accepted visual plan](visual-upgrade-plan.md). Detailed original Blender assets, generated physical material inputs, cached neighborhood reflections, native-resolution FXAA, SA-inspired HUD and live entry screen are implemented. Standard is the new-install default; saved Low remains respected. Four independent art reviews retain all three shape gates; latest driving/pedestrian/street scores are 4.7/4.6/4.4, below the requested 8. A further structural asphalt/terrain/vegetation/hero pass is ready for live inspection. The focused two-player walking/chat/collision/loop/role-swap test passed with maximum observed settled server error 0.0034 units. Complete final-asset acceptance is pending; historical results below do not certify this revision. The user reports that the game runs fine on their machine; this is useful qualitative confirmation, not a measured hardware-GPU benchmark.
