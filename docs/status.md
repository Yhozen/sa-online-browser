...
Dream-loop implementation is in progress; see [accepted visual plan](visual-upgrade-plan.md). Original detailed Blender assets, generated physical inputs, cached neighborhood reflections, native-resolution antialiasing, SA-inspired HUD and live entry screen are implemented. Standard is the new-install default; saved Low remains respected. Six completed independent art reviews retain all three shape gates; latest driving/pedestrian/street scores are **4.8/4.8/4.9**, below the requested 8. The seventh review is evaluating fuller volumetric foliage, coverage-preserving texture sampling, wider directional shadows and refined human/car geometry. Functional regressions now include fixed-step animation and native-DPR radar checks. A fresh complete suite is running; the preceding run passed its 606-second neighborhood soak but failed jump presentation, a premature memory baseline, and yard cases that inherited Standard. Those issues have targeted fixes, not a current all-pass claim. Historical results below do not certify this revision. The user reports that the game runs fine on their machine; this is useful qualitative confirmation, not a measured hardware-GPU benchmark.

### Historical native-resolution correction

Updated 2026-09-08. **Both graphics presets now render at full native screen resolution, per the user's correction.** See [current rendering verification](resolution-results.md). Current checks: four headed graphics scenarios, type checking, build and actual desktop verification passed. The user revised the cloud target to 4 FPS; the existing full-resolution measurements of 5.0 / 4.6 FPS satisfy that target. The threshold was checked against those recorded measurements without rerunning the unchanged renderer. The original neighborhood acceptance and reduced-resolution performance figures below are historical; the complete multiplayer soak suite has not been rerun for this rendering-only correction. See [measured neighborhood results](neighborhood-results.md), [source-hashed verification](neighborhood-verification.json), and the [accepted plan](neighborhood-plan.md). SA-MP 0.3.7 through separate native workers remains the protocol architecture.

Historical neighborhood acceptance: **14 headed browser scenarios**, 3 gateway tests, 6 scene tests, 3 native tests and 2 supervisor probes passed. The final neighborhood soak completed 92 active rounds in 603.1 seconds; the yard completed 78 rounds in 602.4 seconds. Both role assignments completed the neighborhood loop; all reconnect/cleanup and 0.5-unit agreement gates passed. Actual cloud desktop verification also passed.
...
**2026-09-08, next visual pass:** smooth coupe sheet contours and shared normals, coherent oak canopy normals, denser original yard vegetation, and a corrected thin-leaf shader are implemented. The previous shader fed wrapped incidence into specular reflection; the corrected static probe's peak linear RGB is 1.253 (formerly 3,500 in the committed cache), with a regression bound on the exported data. Native-size player/vehicle prewarming now runs before join. Fifth independent visual review remained 4.8/10 in all views; the >=8 art goal is still unmet. The second full verification attempt passed seven browser cases, including both neighborhood routes and twenty reconnects, but was interrupted during the soak after a player disconnected. Final acceptance is pending; older complete neighborhood reports must not be read as proof for this visual revision. The user reports the game runs well on their own machine; this is qualitative confirmation, not a measured hardware-GPU benchmark.
