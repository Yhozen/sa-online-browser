...
The final full headed suite is running on the integrated assets. Exported-asset/clearance checks have passed, but historical results below do not certify this revision. Earlier diagnostic runs led to fixed-step animation, correct native-DPR radar/viewport updates, a completed-frame memory baseline and explicit Low functional fixtures without changing resolution or the 0.5-unit/one-second agreement threshold. The user reports that the game runs fine on their machine; this is qualitative confirmation, not a measured hardware-GPU benchmark.
