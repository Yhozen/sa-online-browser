...
Updated 2026-09-20. The original research roadmap below remains historical context. The browser/open.mp PoC and Arroyo neighborhood are implemented; the [shared activity and original asset milestone](working-game-plan.md) is in final verification. Consult [project status](status.md) and [activity results](working-game-results.md) for measured evidence rather than treating the original gates below as current status.

## Next implementation slices after Arroyo

1. **Shared checkpoint challenge — implemented, final verification in progress.** Arroyo Loop has a manifest-defined course, Pawn-owned countdown/generation/checkpoint order/timing, shared crews, cancellation and top-five server-session records. Native edge-case tests and two keyboard-driven crew laps pass; final retained regression and visual evidence are tracked in the activity results. The browser HUD cannot award a finish. Browser-owned arcade movement remains a prototype, without an anti-cheat claim.
