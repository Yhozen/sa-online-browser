...
The VM's SwiftShader backend is substantially slower than a hardware GPU. Following the user's September 8 correction, both presets use full viewport resolution at the uncapped native device pixel ratio. There is no dynamic resolution scaling. Low mode uses baked vertex shading, simplified fence wires, no shadow maps and no backdrop blur. The earlier reduced-resolution performance result is historical and does not describe the current renderer. The user revised the cloud target to 5 FPS; p95 frame time remains reported without the incompatible former 100 ms gate. Hardware GPU performance remains unmeasured. Standard mode uses PBR materials and soft directional shadow maps. Repeated props/vegetation are instanced; houses are merged by shared material after placement, and coupe body parts share material batches while retaining separate wheels and seat anchors. This keeps downloadable assets and draw calls within the initial budgets.
