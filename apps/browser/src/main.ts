...
import {createCharacter,animateCharacter,disposeActor,createCar,animateCar} from './characters';
import {CollisionIndex} from './collision';
import {FollowCamera} from './camera';
import {mountHUD,minimap} from './hud';
let arena=yard as unknown as SceneManifest, sceneReady=false;
...
function capsule(variant:number){const group=createCharacter(variant);applyQuality(group,quality==='low');scene.add(group);return group;}
let selfMesh=new THREE.Group();
selfMesh.visible=false;
function makeCar(){const group=createCar();applyQuality(group,quality==='low');scene.add(group);return group;}
...
function setStatus(value: string, connected = false) { document.body.classList.toggle('playing',connected); status = value; el('status').textContent = value; el('connection').dataset.connected = String(connected); el<HTMLButtonElement>('join').disabled = !sceneReady || connected || value === 'Connecting…'; el('join-panel').classList.toggle('hidden', connected); el('disconnect').classList.toggle('hidden', !connected); el<HTMLInputElement>('chat-input').disabled = !connected; }
...
function removeMesh(mesh:THREE.Group){scene.remove(mesh);disposeActor(mesh); /* Geometry/materials belong to the shared asset cache. */}
...
function disconnect(reason: string) { terminalReason = reason; clearWorld(); setStatus(reason); el<HTMLButtonElement>('join').disabled = !sceneReady; el('join-panel').classList.remove('hidden'); }
function ensurePeer(id: number) { let p = peers.get(id); if (!p) {
    const label = document.createElement('div');
    label.className = 'player-label';
    document.body.append(label);
    p = { id, name: names.get(id) ?? `Player ${id}`, state: { position: [0, 0, 10], rotation: [1, 0, 0, 0], velocity: [0, 0, 0], mode: 'onFoot', vehicleId: 0, seat: 0, keys: 0 }, streamed: false, mesh: capsule(id % 2), label };
...
function handle(m: ServerMessage) {
    ...
    switch (m.type) {
        case 'init':
            ...
            removeMesh(selfMesh);selfMesh=capsule(self.id??0);selfMesh.visible=false;
...
function collision(x:number,y:number,radius:number){return collisionIndex.collides(x,y,radius);}
...
function step(dt: number) {
    ...
    if (self.mode === 'onFoot') {
        const inputX=Number(keys.has('KeyD'))-Number(keys.has('KeyA')),inputY=Number(keys.has('KeyW'))-Number(keys.has('KeyS'));
        const [x,y]=arena.id==='yard'?[inputX,inputY]:followCamera.movement(inputX,inputY), length=Math.hypot(x,y)||1;
...
function frame(now: number) {
    ...
    const delta = Math.max(0, Math.min(elapsed / 1000, .1));
    previous = now;
    accumulator += delta;
    ...
    animateCharacter(selfMesh,self,delta,arena.groundZ,vehicles.get(self.vehicleId)?.mesh);
    ...
    if(arena.id==='yard'){cameraTarget.lerp(target, 1 - Math.exp(-delta * 7));
    ...
    if(arena.id==='neighborhood')followCamera.update(target.toArray(),heading,self.mode!=='onFoot',delta,collisionIndex);
    if(!document.hidden)renderer.render(scene, camera);
    if(sceneReady){frameTimes.push(elapsed);if(frameTimes.length>1800)frameTimes.shift();}
    if (now > toastUntil)
...
function snapshot() { return JSON.parse(JSON.stringify({ version: 1, scene: {id:arena.id,revision:arena.revision,ready:sceneReady}, graphics:{preset:quality,triangles:renderer.info.render.triangles,calls:renderer.info.render.calls,assets:assetStats,frameTimes,camera:{position:camera.position.toArray(),yaw:followCamera.yaw,obstruction:followCamera.obstruction}}, presentation:{self:{visible:selfMesh.visible,animation:selfMesh.userData.animation,position:selfMesh.position.toArray()},peers:[...peers.values()].map(p=>({id:p.id,visible:p.mesh.visible,animation:p.mesh.userData.animation,position:p.mesh.position.toArray()}))}, status, epoch, controlRevision, self, peers: [...peers.values()].map(p => ({ id: p.id, name: p.name, ...p.state })), vehicles: [...vehicles.values()].map(v => ({ id: v.id, model: v.model, position: v.position, rotation: v.rotation, velocity: v.velocity })), chat: chats, received, sentSequence: sequence, serverSequence: lastServerSequence, rates: { onFootRate, inCarRate } })); }
...
function setQuality(value:string){quality=value==='standard'?'standard':'low';localStorage.setItem('poc-quality',quality);document.body.dataset.quality=quality;renderer.setPixelRatio(quality==='low'?.4:Math.min(devicePixelRatio,1.5));renderer.shadowMap.enabled=quality==='standard';renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1.05;el<HTMLSelectElement>('quality').value=quality;applyQuality(scene,quality==='low');}
el<HTMLSelectElement>('quality').onchange=()=>setQuality(el<HTMLSelectElement>('quality').value);
setQuality(quality);
async function initializeScene(){
 try{
  ...
  sceneReady=true;el<HTMLButtonElement>('join').disabled=false;
 }catch(error){el('loading').textContent=String(error instanceof Error?error.message:error);el('retry-assets').classList.remove('hidden');}
}
