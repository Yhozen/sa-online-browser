...
## Fine start paint, September 20 dream-loop refinement

`assets/textures/arroyo-start-paint.png` and its identical runtime copy are an original built-in image-generation output, distributed under the repository's GPL-3.0-or-later approach to the extent copyright applies. Accepted output: `exec-3c54cedc-851a-4c92-8787-0dbd847822b6.png`. No external image API or GTA texture was used. The source is RGBA 1752×897 with transparent padding; runtime UVs select the two-row twenty-check stripe (X 8–1740, Y 353–536) onto the unchanged 8.4×0.84m start footprint. The original source is retained unchanged. The shared asset loader verifies its checksum, reports progress and owns the texture; the activity owns only the plane and material. It is cosmetic and cannot score a checkpoint.

Prompt: “Create an original game texture asset: a straight rectangular painted race start/finish checker stripe, orthographic directly overhead, no perspective. Exact canvas aspect ratio 10:1, ideally 2000x200 pixels. Exactly TWO rows of TWENTY equal square checks across the width, alternating warm ivory road paint and dark charcoal road paint. Outer silhouette a plain straight rectangle. Realistic aged painted asphalt marking with tiny irregular chips, fine pinholes, rubbed/faded edges and subtle internal opacity wear. Chips generally 1–3 pixels, a few 5 pixels, no large missing square blocks. All spaces where paint is chipped must be truly transparent alpha, no asphalt photo/background, no drop shadow, no lighting, no gradient background. Cream squares should remain mostly solid (about 92% coverage), dark squares slightly faded charcoal. Color-only texture, uniformly lit, readable at a distance. No text, numbers, icon, border, banner poles, cloth, waving flag or extra elements. Production-ready original PBR game albedo decal, restrained variation rather than gritty noise.”

The returned padded canvas differs from the requested aspect ratio, so UV selection preserves the exact original stripe without stretching the full canvas. This replaces the visibly square 5cm wear cells identified by the first fresh art review. New texture allocation is approximately 8MiB including generated mipmaps; native output resolution is unchanged.

## Original field yucca, September 20 landscaping refinement

`activity-yucca` responds to the dream-loop review's request for distinct rosettes in the existing planted beds. Its source is the appended builder in `tools/assets/activity-kit.py`; the editable `assets/source/activity-yucca.blend` retains **43 named components**. The runtime export is `apps/browser/public/assets/activity-yucca.glb`. All geometry and material definitions are original and use **GPL-3.0-or-later**. No additional bitmap, image-generation service, downloaded plant, external font or new runtime texture is needed.

The plant has **33 individually curved, thin solid leaves** in four growth rings, narrow muted waxy margins, varied dry terminal tips, a compact upright heart, a low woody crown and **nine bent dry basal leaves**. The first authoring view exposed visibly coarse blade bends and overly uniform yellow tips; the accepted revision uses fourteen longitudinal sections per blade, asymmetric sweeps, selective terminal weathering and a smaller crown. Existing `activity-agave`, `activity-agave-edge` and `wood` material definitions are reused. The full silhouette exists in geometry under both graphics presets.

The local bounds are X **−0.441235–0.435534**, Y **−0.441213–0.462230**, Z **0–0.649675** meters. The maximum horizontal vertex radius is **0.474223 m**, so a centered **0.98×0.98×0.67 m** manifest box encloses the entire plant at every Z-axis rotation; placement excludes roads, sidewalks and fences. The source has no baked ground disc. Actual placement/collision and scene revisions remain in the shared manifest.

The export contains **9,872 triangles**, **three material primitives** and **524,796 bytes**, below its standalone 10,000-triangle/600,000-byte authoring envelope. The five-model kit now totals **2,997,876 bytes**, **40,988 triangles** and **23 primitives** before placement instancing. All four previous exports and editable sources remain unchanged. A repeated pinned-Blender standalone export reproduces the yucca GLB SHA256; normal setup consumes the committed export.

Five exported-data tests pass. In addition to shared finite position/normal/UV/color and metric bounds checks, the yucca test verifies every vertex fits the full rotated collision envelope, solid leaf materials need no alpha sorting or new image, and a ray intersects the upright green heart. The isolated Cycles authoring preview and report are `.dream-loop/activity-assets/activity-yucca.png` and `yucca-authoring-audit.json`; they are **not live gameplay evidence**. Integrated street views and gameplay verification are recorded separately by the parent milestone.
