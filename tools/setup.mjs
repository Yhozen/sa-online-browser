...
// Container images install these in the image build instead; see Dockerfile.
if (process.env.POC_SKIP_SYSTEM_DEPS !== '1') run('sudo', ['dnf', 'install', '-y', 'gcc', 'gcc-c++', 'clang', 'cmake', 'ninja-build', 'make', 'xorg-x11-server-Xvfb', 'xorg-x11-xauth']);
run('npm', ['ci']);
...
// The server image renders nothing; only in-container verification needs Chromium.
if (process.env.POC_SKIP_PLAYWRIGHT !== '1') run('npx', ['playwright', 'install', 'chromium']);
